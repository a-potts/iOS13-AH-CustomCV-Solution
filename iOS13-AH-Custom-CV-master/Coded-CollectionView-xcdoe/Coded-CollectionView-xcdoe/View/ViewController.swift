...
func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
   ...
   func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
           if segue.identifier == "ViewYachtSegue" {
               guard let indexPath = collectionView.indexPathsForSelectedItems?.first?.item,
                   let serviceVC = segue.destination as? YachtDetailViewController else {return}
               
            let selectedYacht = data[indexPath]
            serviceVC.megaYacht = selectedYacht
           }
       }
    
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
           performSegue(withIdentifier: "ViewYachtSegue", sender: indexPath)
       }
...
class CustomCell: UICollectionViewCell {
    ...
    fileprivate let bg: UIImageView = {
        let iv = UIImageView()
        iv.image = #imageLiteral(resourceName: "Y4")
